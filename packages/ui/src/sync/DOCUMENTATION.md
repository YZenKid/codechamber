...
12. Slash commands use `session.command` so OpenCode expands their templates. Slash skills use the optimistic prompt path with native skill attachments, preserving the original text and inline skill mentions. A cached command takes precedence over a same-name skill in the session's directory. Both routes carry files. `session.command` admits attached context, including session knowledge, as synthetic messages before sending; skills use the prompt route because native skill attachments are unavailable on `session.command`.
13. Stopping a run aborts the addressed session first and then every known descendant that is live-active. Each abort carries that session's own resolved directory, because a per-session abort sent to the wrong per-directory instance cancels nothing while still succeeding. Idle and `unknown` descendants are left running, a descendant abort failure is ignored so its siblings still run, and live activity is re-read per abort so a descendant that turns active after the subtree snapshot is still stopped.
